...
- **Port — two modes** (master switch `demote_paused`, default on; `--no-demote` ablates):
  - **`on-demand` (DEFAULT, `demote_ondemand=True`)** — lazy/minimal. Preserved API-waiting requests keep their KV until a new/resuming request **fails `allocate_slots`**; then `_reclaim_ondemand` frees only the **minimum** number of preserved-paused requests (lowest-`arrival_waste` first, so the costliest-to-redo stays pinned longest) needed to cover that **chunked** admission, and defers one step. Runs under any running count (parked `WAITING_FOR_STREAMING_REQ` holders are invisible to vLLM's native preemption). No vLLM core edit. Demoted requests reload only at their API resume.
  - **`proactive` (`--demote-proactive`, faithful to the original)** — the `schedule()` pre-pass `_mars_demote_paused`: every step, `max_waste` over **all** paused-preserve requests, demote every demotable one with `waste < max_waste`; costliest stays pinned.
- **Faithful**: the `proactive` mode matches the original's per-step "keep only max-waste preserved" cadence. The `on-demand` default is a **deliberate v1 improvement** (the original had no reason to be lazy — it lacked v1's write-through CPU mirror that makes a swap-out free).
- **Empirical (A40 / Qwen2.5-14B, qps 3–13, norm-latency ms/tok):** `on-demand` is the **best policy across the whole sweep** — dormant at low load (qps3 86 ≈ `--no-demote` 86; 0 reclaims through qps7) and the clear winner at saturation (qps13 **2525** vs no-demote 4901 vs Swap 6448 vs proactive 8770), draining 100%. It beats `--no-demote` (which starves new requests behind preserved KV) **and** avoids the `proactive` churn that melts down (qps9 proactive 11.3 s/tok @ 89% drained vs on-demand 0.20 s/tok @ 100%). Rejected alternatives (removed): a usage-floor pressure gate on `proactive` (mitigated but still lost to `--no-demote`) and a mid-wait preload (inert under write-through caching). A prefill microbench confirmed concurrent KV copies cost only 2–5%/iter, so demotion's harm was **admission queueing**, which on-demand minimises by freeing just-in-time and minimally.
...
- **v1 re-derivation (deliberate)**: the `_v2_score` memory-time (`before`/`after`/recompute/preserve) now uses the **profiled** forward coefficients (`cost_a`/`cost_c`), not the original hand-tuned `a=0.1, c=10` — so the queue score is on the **same calibrated scale** as `cost_model`. The **swap branch** is re-derived for the write-through cache: the original `before + swap + api + swap + after` charged swap-OUT **and** swap-IN; v1's write-through mirror makes swap-out free, so the first `swap` term is dropped and the swap-IN is priced by the **same `cost_model.swap_waste`** used for classify/demotion (`before + api·0.1 + swap_waste + after`). This resolves the prior inconsistency (classify used the v1 swap price, ordering used the V0 two-transfer price). **A/B (qps 3–13, vs the committed V2)**: arrival-strategy mix unchanged (ordering-only refinement); neutral at qps 3–11 (±1.6%) and a win at saturation (qps13 E2E −7%, norm-latency −15%, 100% drain).
